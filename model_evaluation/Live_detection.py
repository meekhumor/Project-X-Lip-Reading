...
def face_extractor(img):
    ...
    face_classifier = cv2.CascadeClassifier('dataset_preprocessing/assets/haarcascade_frontalface_default.xml')
...
def lips_extractor(img):
    predictor = dlib.shape_predictor('dataset_preprocessing/assets/shape_predictor_68_face_landmarks.dat')
...
while True:
    ...
    if recording:
        frames.append(frame)  
        cv2.putText(display_frame, "Recording...", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)
    else:
        cv2.putText(display_frame, "Press Space to Record", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
...
# Load the model
loaded_model = tf.keras.models.load_model('model_architecture/Saved Model/3D_CNN_LSTM_words_final.h5')
